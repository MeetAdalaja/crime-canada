...
class ModelStore:
    def __init__(self):
        ...
        self.meta = {}
        self.df_ON = None
        self.backtests = {}  # {violation: { "2021": {...}, "2022": {...}, "2023": {...} }}
        self._forecast_cache = {}  # (violation,to_year)->dict
        self._load_data()
        self._load_models()
    ...
    def _load_models(self):
        for v in VIOLATIONS:
            slug = self._slug(v)
            mp = MODELS_DIR / f"model_{slug}.joblib"
            mt = MODELS_DIR / f"meta_{slug}.json"
            if mp.exists():
                self.models[v] = joblib.load(mp)
            if mt.exists():
                with open(mt) as f:
                    self.meta[v] = json.load(f)
    ...
    def forecast_to_year(self, violation: str, to_year: int):
        ...
        res = {"forecast": yhat, "from_year": last_year+1, "to_year": to_year}
        self._forecast_cache[key] = res
        return res

    def predict_specific_year(self, violation: str, year: int):
        """
        Serve *precomputed* backtests only (2021–2023).
        No retraining happens at request-time.
        """
        bt = self.backtests.get(violation, {})
        rec = bt.get(str(int(year)))
        if rec:
            return {
                "year": int(rec["year"]),
                "yhat": float(rec["yhat"]),
        ...
        # If not available (e.g., year outside 2021–2023), return actual (if exists) without yhat
        sub = self.df_ON[self.df_ON["Violations"] == violation]
